...
class CardPredictor:
    ...
    def train_svm(self):
        ...
        # 识别中文
        self.model_Chinese = SVM(C=1, gamma=0.5)
        ...
        if os.path.exists("svmchinese.dat"):
            self.model_Chinese.load("svmchinese.dat")
        else:
            chars_train = []
            chars_label = []
            ...
            self.model_Chinese.train(chars_train, chars_label)

    def save_traindata(self):
        if not os.path.exists("svm.dat"):
            self.model.save("svm.dat")
        if not os.path.exists("svmchinese.dat"):
            self.model_Chinese.save("svmchinese.dat")
    ...
    def predict(self, car_pic, resize_rate=1):
        colors, card_imgs = self.predict_plate_image(car_pic, resize_rate)
        for i, color in enumerate(colors):
            if color in ("blue", "yellow", "green", "white", "black"):
                predict_result, roi, card_color, flag = self.predict_plate_value(color, card_imgs[i])
                if flag:
                    break
        return predict_result, roi, card_color  # 识别到的字符、定位的车牌图像、车牌颜色

    # 使用传统方法识别出图片中的车牌
    def predict_plate_image(self, car_pic, resize_rate=1):
        ...
        print("h, w:", pic_height, pic_width)
        blur = self.cfg["blur"]
        # 高斯去噪
        if blur > 0:
        ...
        # 进行各类形态学的变化：op =  cv2.MORPH_OPEN 进行开运算，指的是先进行腐蚀操作，再进行膨胀操作
        #                   op = cv2.MORPH_CLOSE 进行闭运算， 指的是先进行膨胀操作，再进行腐蚀操作
        img_opening = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel)
        # cv2.imwrite('./img_open.png', img_opening)
        # 将两幅图像合成为一幅图像
        ...
        # cv2.imwrite('./img_edge1.png', img_edge1)
        ...
        for card_index, card_img in enumerate(card_imgs):
            ...
            if yellow * 2 >= card_img_count:
                ...
                color = "bw"
            # elif black * 2 >= card_img_count:
            #     color = "black"
            # elif white * 2 >= card_img_count:
            #     color = "white"
            # print(color)
            colors.append(color)
    ...
    # 识别出车牌中的字符
    def predict_plate_value(self, color, card_img):
        # 以下为识别车牌中的字符
        predict_result = []
        roi = None
        card_color = None
        ...
        if len(wave_peaks) == 0:
            print("peak less 0:")
            return predict_result, roi, card_color, 0
        # 认为水平方向，最大的波峰为车牌区域
        ...
        for i, wave in enumerate(wave_peaks):
            # cv2.imread('./{0}.jpg'.format(i), wave)
        ...
        for i, part_card in enumerate(part_cards):
            ...
            if i == 0:
                resp = self.model_Chinese.predict(part_card)
                # cv2.imwrite('./chinese.jpg', part_card)
                charactor = provinces[int(resp[0]) - PROVINCE_START]
            else:
                # cv2.imwrite('./{0}.jpg'.format(i), part_card)
        ...
        roi = card_img
        cv2.imwrite('test/card_img.jpg', card_img)
        card_color = color
...
if __name__ == '__main__':
    ...
    c.train_svm()
    # 识别到的字符、定位的车牌图像、车牌颜色
    r, roi, color = c.predict("./test/2.jpg")
    print("r:", r)
    print("color:", color)
