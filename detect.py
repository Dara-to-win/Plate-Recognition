...
def show_results(img, xywh, conf, landmarks, class_num):
    ...
    cv2.rectangle(img, (x1, y1), (x2, y2), (255, 255, 255, 0.1), thickness=tl, lineType=cv2.LINE_AA)
...
def detect(model, image_path, device, output_path):
    # Load model
    img_size = 640
    conf_thres = 0.3
    iou_thres = 0.5
    ...
    for i, det in enumerate(pred):  # detections per image
        ...
        if len(det):
            ...
            for j in range(det.size()[0]):
                ...
                # cv2.imwrite('./test_result.jpg', orgimg)
                ...
                cv2.imencode('.jpg', dst)[1].tofile(output_path)
...
if __name__ == '__main__':
    ...
    image_path = './test/car5.jpg'
    output_path = './test/car5_warp.jpg'
    detect(model, image_path, device, output_path)
    print('over')
